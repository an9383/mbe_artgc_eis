...
namespace KR.MBE.CommonLibrary.Handler
{
        ...
        */
        ...
        private static void ReadConfigFile()
        {
            if( m_Middleware == MiddlewareType.NONE )
            {
                ...
                if (htConfig["Middleware"] == null)
                {
                    LogManager.Instance.Error("Middleware Type is not defined in the config file - set default activemq");
        ...
        */
        ...
        private static DataSet sendMessage_ACTIVEMQ( string strMessageName, Hashtable htBody, string strDataListTag, string strDataID, string strDataInfoTag, ArrayList arrBindValue )
        {
            ...
            using (Util.BeginBusyBlock())
            {
                ...
                if (strHeader == string.Empty)
                {
                    LogManager.Instance.Error("make XML Header Message. Please Check the middleware config file");
                    return dsReturn;
                }
        ...
        public static void SendMessageAsyncToMonitoringECS(string strTransactionID, string strBody)
        {
            string strHeader = string.Empty;
            ...
            try
            {
                ...
                if (m_Middleware == MiddlewareType.ACTIVEMQ)
                {
                    m_oSender_ActiveMQ.SendTopicToMonitoringECS(strSendMessage);
                }
            }
            catch (Exception ex)
            {

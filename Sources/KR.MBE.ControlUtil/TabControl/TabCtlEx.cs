...
namespace KR.MBE.UI.ControlUtil
{
    ...
    public class TabCtlEx : System.Windows.Forms.TabControl
    {
        ...
        private const int g_ItemWidth = 100;
        private const int g_ItemHeight = 24;
        ...
        // TabPage Close 시 Confirm 을 할지 여부
        ...
        private void InitializeComponent()
        {
            ...
            this.ItemSize = new System.Drawing.Size( g_ItemWidth, g_ItemHeight );
        ...
        protected override void OnDrawItem( DrawItemEventArgs e )
        {
            if( e.Bounds != RectangleF.Empty )
            {
                ...
                for( int nIndex = 0; nIndex < this.TabCount; nIndex++ )
                {
                    ...
                    tabTextArea.Y = tabTextArea.Y + 5;
                    tabTextArea.X = tabTextArea.X - 10;
        ...
        {
            /*
            TabPageEx oPages = (TabPageEx)this.TabPages[nIndex];
            if (oPages.Menu != null)
            {
                return true;
            }
            ...
            if( e.Button == MouseButtons.Left )
            {
                if( !DesignMode )
                {
                    ...
                    if( tabTextArea.Contains( pt ) )
                    {
                        if( confirmOnClose )
                        {
                            //string sMessage = "You are about to close [" + this.TabPages[SelectedIndex].Text.TrimEnd() + "] tab. \n\n" + "Are you sure you want to continue?";
                            string sMessage = this.TabPages[SelectedIndex].Text.TrimEnd() + "\n\n" + "위 화면을 종료하시겠습니까?";
                            //if( MessageBox.Show( sMessage, "확인", MessageBoxButtons.YesNo ) == DialogResult.No )
                            if (MessageBox.Show(sMessage, "Check", MessageBoxButtons.YesNo) == DialogResult.No)
